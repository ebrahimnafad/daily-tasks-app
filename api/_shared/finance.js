...
export async function handleFinance(req, res, finRes) {
  ...
  try {
    ...
    if (method === 'GET') {
      ...
      switch (finRes.table) {
        case 'finance_income':
          rows = await sql`SELECT * FROM finance_incomes WHERE user_id = ${userId}`;
          data = rows.map((r) => ({
            id: r.id,
            title: r.title,
            icon: r.icon,
        ...
        case 'finance_obligations':
          rows = await sql`SELECT * FROM finance_expenses WHERE user_id = ${userId}`;
          data = rows.map((r) => ({
            id: r.id,
            ...
            isActive: r.is_active,
            dueDay: r.due_day,
            quarterMonth: r.quarter_month,
            totalAmount: r.total_amount,
            totalInstallments: r.total_installments,
            endDate: r.end_date,
            seasonMonth: r.season_month,
            monthlySetAside: r.monthly_set_aside,
            notes: r.notes,
            createdAt: r.created_at?.toISOString() || new Date().toISOString(),
            updatedAt: r.updated_at?.toISOString() || new Date().toISOString(),
          }));
        ...
        case 'finance_payments':
          rows = await sql`SELECT * FROM finance_transactions WHERE user_id = ${userId}`;
          data = rows.map((r) => ({
            id: r.id,
            expenseId: r.expense_id,
            categoryId: r.category_id,
            amount: r.amount,
            date: r.transaction_date?.toISOString() || new Date().toISOString(),
            status: r.status,
            notes: r.notes,
          ...
          break;
        case 'finance_goals':
          rows = await sql`SELECT * FROM finance_goals_rel WHERE user_id = ${userId}`;
          data = rows.map((r) => ({
            id: r.id,
            title: r.title,
            icon: r.icon,
        ...
        case 'finance_categories':
          rows = await sql`SELECT * FROM finance_categories WHERE user_id = ${userId}`;
          data = rows.map((r) => ({
            id: r.id,
            title: r.title,
            icon: r.icon,
    ...
    if (method === 'POST') {
      ...
      await sql.begin(async (t) => {
        ...
        if (finRes.table === 'finance_income') {
          if (itemIds.length > 0)
            await t`DELETE FROM finance_incomes WHERE user_id = ${userId} AND id != ALL(${itemIds})`;
          ...
          if (itemIds.length > 0)
            await t`DELETE FROM finance_expenses WHERE user_id = ${userId} AND id != ALL(${itemIds})`;
          ...
          if (itemIds.length > 0)
            await t`DELETE FROM finance_transactions WHERE user_id = ${userId} AND id != ALL(${itemIds})`;
          ...
          if (itemIds.length > 0)
            await t`DELETE FROM finance_goals_rel WHERE user_id = ${userId} AND id != ALL(${itemIds})`;
          ...
          if (itemIds.length > 0)
            await t`DELETE FROM finance_categories WHERE user_id = ${userId} AND id != ALL(${itemIds})`;
