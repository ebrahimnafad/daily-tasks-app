...
export default function GoalsSection({ goals, setGoals }) {
  ...
  const save = useCallback(() => {
    ...
    const monthsLeft = deadlineDate
      ? Math.max(1, Math.ceil((deadlineDate - now) / (1000 * 60 * 60 * 24 * 30.44)))
      : 0;
  ...
  return (
    <section className="fin-section" aria-label="أهداف الادخار">
      ...
      {goals.map((goal) => {
        ...
        const monthsLeft = deadlineDate
          ? Math.max(0, Math.ceil((deadlineDate - new Date()) / (1000 * 60 * 60 * 24 * 30.44)))
          : null;
          ...
          <div key={goal.id} className="fin-goal-card">
            ...
            <div className="fin-goal-card__meta">
              ...
              {goal.monthlyTarget > 0 && (
                <span>الاقتطاع: {goal.monthlyTarget.toLocaleString('ar-SA')} ر.س/شهر</span>
              )}
            </div>
            ...
            {form.targetAmount &&
              ...
              (() => {
                const months = Math.max(
                  1,
                  Math.ceil((new Date(form.deadline) - new Date()) / (1000 * 60 * 60 * 24 * 30.44))
                );
