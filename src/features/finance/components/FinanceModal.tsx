...
import type { Obligation, FinanceModalState } from '../types';
...
interface FinanceModalProps {
  ...
  onSave: (data: Partial<Obligation>, mode: 'add' | 'edit', id?: string | number) => void;
  onClose: () => void;
}

export default function FinanceModal({ modal, onSave, onClose }: FinanceModalProps) {
  const [form, setForm] = useState<Record<string, any>>(() => {
    if (!modal) return EMPTY;
    if (modal.mode === 'edit' && modal.data) {
      const d = modal.data as any; // Type workaround for dynamic properties not strictly modeled
      return {
        icon: d.icon || '📋',
        title: d.title || '',
  ...
  const save = useCallback(() => {
    ...
    const data: any = {
      ...
      isVariable: form.type === 'variable',
      totalAmount: form.type === 'installment' ? Number(form.totalAmount) || 0 : undefined,
      totalInstallments:
        form.type === 'installment' ? Number(form.totalInstallments) || 0 : undefined,
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={modal.mode === 'add' ? 'إضافة التزام' : 'تعديل التزام'}
    >
      <div className="cbox fin-modal fin-modal--lg">
        <h3 className="fin-modal__title">
          {modal.mode === 'add' ? 'إضافة التزام مالي' : 'تعديل التزام'}
        </h3>
        ...
        <div className="fin-modal__scroll">
          ...
          <label className="fin-label">
            {form.type === 'variable' ? 'المبلغ التقديري' : 'مبلغ الدفعة'} (ر.س)
            <input
              className="fin-input"
              type="number"
              min="0"
